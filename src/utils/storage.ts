...
export async function readData<T>(filename: string): Promise<T | null> {
  try {
    ...
    // SECURITY: Prevent leaking absolute OS paths and usernames from Tauri raw filesystem errors
    console.error(`[storage] Failed to read "${filename}": (Error details omitted for security)`);
    return null;
  }
}
...
export async function writeData<T>(filename: string, data: T): Promise<void> {
  try {
    ...
    // SECURITY: Prevent leaking absolute OS paths and usernames from Tauri raw filesystem errors
    console.error(`[storage] Failed to write "${filename}": (Error details omitted for security)`);
  }
}
